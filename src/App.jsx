...
const APPROVERS = {
  ...
  INGENIERIA: ['Subgerente Comercial', 'Gerente Comercial', 'Gerente General', 'Gerente de I+D y Producción', 'Gerente de Operaciones'],
  OPERACIONES: ['Gerente de I+D y Producción', 'Subgerente Comercial', 'Gerente Comercial', 'Gerente General'],
        ...
        <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-fade-in border border-slate-200">
          ...
          <div className="px-8 pb-8">
            ...
            <div className="mt-8 pt-6 border-t border-slate-100">
              <p className="text-xs text-slate-400 text-center mb-3">Usuarios habilitados (Clave por defecto: <strong className="text-slate-600">123</strong>):</p>
              <div className="flex flex-wrap gap-1.5 justify-center">
                {Object.keys(usersDb).map(u => (
                  <span key={u} className="bg-slate-50 text-slate-500 text-[10px] px-2 py-1 rounded border border-slate-200 font-medium cursor-help hover:bg-[#899264] hover:text-white transition-colors" title={usersDb[u].name}>{u}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          {(view === 'dashboard' || view === 'projects') && (
            <div className="max-w-5xl mx-auto animate-fade-in">
              <div className="flex justify-between items-center mb-6">
                ...
                {['Administrador del sistema', 'Gerente Comercial', 'Subgerente Comercial'].includes(role) && (
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {area.docs.map(doc => {
                            ...
                            const canEditDeadline = ['Gerente General', 'Subgerente Comercial', 'Administrador del sistema'].includes(role);
